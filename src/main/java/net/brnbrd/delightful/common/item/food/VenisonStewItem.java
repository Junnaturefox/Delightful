...
public class VenisonStewItem extends DConsumableItem {
	...
	public boolean enabled() {
		return this.isDependencyTag() && super.enabled();
	}
	...
	public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> comps, @NotNull TooltipFlag pIsAdvanced) {
		if (super.enabled() && !this.isDependencyTag()) {
			comps.add(Component.translatable("tooltip.requires_tag"));
			comps.add(Component.literal(DelightfulItemTags.RAW_VENISON_COMPAT.location().toString()).withStyle(ChatFormatting.UNDERLINE));
		} else {
